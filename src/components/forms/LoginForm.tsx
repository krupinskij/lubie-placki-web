  ...
  return (
    <Mutation mutation={LOGIN_MUTATION}>
      {(trigger: MutationFunction<any, Record<string, any>>, result: MutationResult<any>) => (
        ...
        >
          {({ isValid }) => (
            <FormContainer title={mini ? '' : 'Zaloguj się'}>
              <FormFields>
                <EmailField label={mini ? '' : 'Podaj email:'} required={true} />
                <PasswordField label={mini ? '' : 'Podaj hasło:'} required={true} />
              </FormFields>
              <FormActions>
                <SubmitButton disabled={!isValid} text="Zaloguj się" />
              </FormActions>
              <FormLink pretext={mini ? 'lub' : 'Nie masz jeszcze konta?'} text="Zarejestuj się" to="/register" />
            </FormContainer>
          )}
        </Formik>
      )}
